...
import { esCorrRolValido } from "../constants/roles";
...
export const authService = {
  async login(
    ...
    // Solo los perfiles habilitados actualmente pueden iniciar sesión (identificados por
    // corr_rol); cualquier otro rol legacy se trata como si el usuario no existiera.
    if (!esCorrRolValido(usuario.corr_rol)) {
  ...
  async getSesionActual(
    ...
    const roles = rolesDe(usuario);
    const permisos = await usuarioSicapRepository.getPermisosDeUsuario(
      usuario.rut_usu,
    );
    return {
      usuario: toSafeUsuarioSicap(usuario.get({ plain: true })),
