...
export const reemplazoBenProyectoService = {
  async listar() {
    return reemplazoBenProyectoRepository.findAll();
  },
  ...
  async crear(data: CrearReemplazoInput, archivos: Express.Multer.File[], rutUsuarioSolicitante: number) {
    const [beneficiarioActual, proyecto] = await Promise.all([
  ...
  async actualizarEstado(id: number, status: ReemplazoStatus) {
